...
class PolicyNet(NeuralNet):
    ...
    def __init__(self, shape, graph=None):
        ...
        with self.graph.as_default():
            ...
            angular = 5. * 10**(-4) * tf.norm(angular)
            linear = 5. * 10**(-4) * tf.norm(linear)
            action = 1. * 10**(-4) * tf.norm(self.action_noise)
            ...
            cf = lambda i, vf: tf.less(i, self.states_f_l-1)
            ...
            vf_w = tf.while_loop(cf, 
                    self.gen_body(self.input[:self.states_f_l],
                        ...
                        0), 
                    [iterf, vf])[1]
            vf_w += DISCOUNT_VALUE**tf.cast(self.states_f_l-1, dtype='float') * self.value_f
           
            # create value function for on policy
            iterp = tf.constant(0, dtype=tf.int32)
            vp = tf.constant(0, dtype=tf.float32)
            cp = lambda i, vp: tf.less(i, self.states_p_l-1)
            ...
            vp_w += DISCOUNT_VALUE**tf.cast(self.states_p_l-1, dtype='float') * self.value_p
            ...
            def f2():
                return self.A
            #self.A = tf.case({tf.less(0., self.A): f1}, default=f2)

            factor = tf.square(tf.norm(self.action_noise - action_pred.__getitem__(self.states_f_l)))
            # calculate gradients of A to and multiply with J (gradients of a to param)
            grads = -tf.divide(tf.multiply(self.A, self.action_noise - action_pred.__getitem__(self.states_f_l)), factor)
    ...
    def gen_body(self, states, actions, offset):
        def body(i, v):
            ...
            angular = 5. * 10**(-4) * tf.norm(states[i][12:15])
            linear = 5. * 10**(-4) * tf.norm(states[i][15:18])
            action = 1. * 10 ** (-4) * tf.norm(actions[i+offset])
